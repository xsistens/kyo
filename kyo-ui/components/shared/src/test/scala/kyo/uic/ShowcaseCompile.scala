...
object ShowcaseCompile:
    ...
    def waveMEnvWiring(confirmOpen: SignalRef[Boolean], notify: Any < Async)(using
        ...
        Env.get[uic.ToastService].map(svc => waveMPage(svc, confirmOpen, notify))

    /** Wave N: the form-input block — Double-typed numeric fields, the masked
    ...
    /** Wave O: the container/data block — panel accordion, list shufflers, media
