...
/** Overlay — the server-honest floating-panel building block (kyo primitive; the
  ...
  * (mounted8 `observeViewportById`) — and pushes the resolved geometry (the
  ...
  *
  * Enter/leave animations (Welle P): the panel fades in on open (a from-state
...
final case class Overlay private (
    ...
    /** Whether the panel measures itself continuously (mounted8 self-addressing:
    ...
    private def openContent(using Frame): UI =
        if portalV then
            ...
            // Self-addressing (mounted8): on open, mint an id + stamp it on the panel,
