...
/** Knob — native kyo-ui, PrimeOne design (mirrors PrimeVue/PrimeReact's Knob
  ...
  * drag: pressing/dragging the dial maps the pointer angle
  ...
  * is pointer-events:none, so per-frame re-render never detaches the
  * capture.
  *
    ...
    private def renderResolved(using Frame): UI =
        valueBinding match
            case Present(ReactiveVariable(ref)) =>
                // Pointer-drag + keyboard. The drag handler sits on the STABLE
                // outer div, never re-rendered, while the reactive svg dial
