...
The module cross-builds for the JVM and Scala.js. There is no Native row and no per-platform component API: every component lives in shared source and renders identically on both. The JS build pulls `scala-java-time` so `DateCodec` has a `java.time` to work against, and ships no timezone database, leaving that bundle-size decision to you.
