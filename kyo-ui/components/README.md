...
Sorting is opt-in per column through `sortBy`, and the header click cycle (ascending, descending, unsorted) needs a `sort` ref on the table to persist. Clicking a second column appends to the spec rather than replacing it, so multi-sort needs no modifier key; once two or more columns are sorted each sorted header shows its rank, because the spec is ordered and the direction icons alone cannot say which key wins. Row expansion pairs `expanded(ref)` with `rowExpansionTemplate`.
