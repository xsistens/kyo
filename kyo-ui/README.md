...
- `.focusAuto(v: Boolean)`, `.focusRestore(v: Boolean)`: declarative focus management. `focusAuto` calls `.focus()` once when a re-render newly inserts the element (like native `autofocus`, but re-render-aware: an echo re-render of an already-visible element never re-steals focus); `focusRestore` returns focus to the element focused before seeding when the seeded element leaves the document (the modal-returns-focus-to-its-trigger pattern), and only where that removal actually took the focus with it: a reader who moved focus themselves first (tabbing out of a panel that then closes) keeps where they moved it. Pair with `tabIndex(-1)` for elements that are not natively focusable.
- `.scrollAuto(v: Boolean)`: declarative scroll-into-view. Whenever a patch leaves the flag on a different element than the one that carried it before, the client scrolls that element into view with `block: "nearest"`, which moves the nearest scrollable ancestor by the least it can and not at all when the element is already visible. It exists for the roving HIGHLIGHT, which the browser does not follow: a moving focus is scrolled to by the browser itself, but a highlight is a class plus `aria-activedescendant` on a container that never moves. Unlike `focusAuto` it re-fires as the flag moves, since that is the movement it follows, and it covers the one moment a `scrollIntoView` command cannot: a panel opening onto a row that does not exist yet when the handler runs.
