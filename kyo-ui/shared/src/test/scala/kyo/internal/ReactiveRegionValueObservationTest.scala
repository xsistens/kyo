...
/** A region over a PROJECTION of a signal, while the signal keeps moving underneath it.
  ...
  * them". Every wait is a paint count, never the wall clock and never virtual time: a region whose loop repaints per emission closes a
  * per-value Scope on each one, which a frozen clock does not let finish.
  */
class ReactiveRegionValueObservationTest extends kyo.test.Test[Any]:

    /** A record with a slice that moves on every emission and one that does not — the playback state in miniature. */
