...
class KeyedRootParityTest extends kyo.test.Test[Any]:
    ...
    /** True when `html` is exactly one logical child that `pathAttr` NAMES: an element whose own tag carries the
      * path (children render at `pathAttr.0`, `pathAttr.1`, … and so never match the quoted form).
      *
      * A region also paints one logical child, a marker pair, but it names itself by its range id rather than by
      * the row path, and the reconciliation matches rows by path. So a region-rooted row is not addressable here
      * even though it is a single root, and the whole-list repaint takes it. That is a narrowing against the
      * fork's own marker foundation, where a span carried the path itself; it costs such a list the structural
