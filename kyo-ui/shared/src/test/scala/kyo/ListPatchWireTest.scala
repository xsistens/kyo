    ...
    "a row that no key can name travels as a whole-list replace" in {
        ...
        withUI(ui) {
            for
                ...
                op <- lastOp
            yield op match
                // An HTML region's whole-list repaint replaces the content between its comment anchors;
                // HtmlOp.Replace is the SVG-only, path-addressed form (UIServer.sendRegion).
                case HtmlOp.ReplaceRange(_, html) =>
                    assert(html.contains("""class="l"""") && !html.contains("\"b\""), s"unexpected payload: $html")
