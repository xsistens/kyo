    ...
    private def normalizeWith(
        ...
        ui match
            ...
            case ui: Element =>
                ...
                for (kids, hdl) <- walkStatic(ui, path, svg, regionIdentity, parentContext, mountDispatch, discoverRootBound = false)
                yield ReactiveUI(
                    path,
                    elementSignal,
                    isConst = isConstNode,
                ...
                for childWalks <- Kyo.foreach(elem.children.toSeq.zipWithIndex) { (child, i) =>
                        ...
                        child match
                            ...
                            case childElem: Element if collectSignalRef(childElem).nonEmpty =>
                                ...
                                for rui <- normalizeWith(childElem, childPath, childSvg, childContext, childParentContext, mountDispatch)
                                yield (Seq(rui), Seq.empty[(Int, Handler)])
                            case _ =>
                                for (innerKids, innerHandle) <- walkStatic(child, childPath, childSvg, childContext, childParentContext, mountDispatch)
    ...
    private def subscribeScoped(
        ...
        rui.mountedSpec match
            ...
            case Absent =>
                ...
                else subscribeRegion(rui, rui.signal, exchange, signalChangeTime, Absent, mountDispatch)
    end subscribeScoped
