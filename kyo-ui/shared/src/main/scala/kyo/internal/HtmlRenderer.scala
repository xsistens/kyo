...
private[kyo] object HtmlRenderer:
    ...
    /** Reserved path segment for a reactive region whose current value is ITSELF a reactive region
      * (`Reactive`/`Foreach`/`Mounted`). Without it, nested reactive wrappers collapse onto one
    ...
    private def clientJsMorph(basePath: String): String =
        s"""(function(){
           ...
           |      // element carrying the path: morph it 1:1 in place. Genuinely unpainted DOM stays a
           |      // silent no-op. Twin of DomBackend.onChange.
           ...
           |    var rr=el?null:__kyoRegions[p];
           |    // A removal is a departure, so leave-ghosts are cloned before the nodes go: from the element
           |    // itself for an element region, from the marker range for a region with no single root.
           |    var __rgh=el?kyoLeavePrepare(el,{}):((rr&&rr.s.isConnected)?__kyoRangeGhosts(rr,{}):[]);
           |    if(el)el.remove();
           ...
           |// Transition and focus-auto bookkeeping used to take a single region root; with markers a region is a
           |// sibling RANGE, so each helper folds over the range's top-level elements (each already covers its own
           |// subtree). Twins of rangePaths/rangeLeaveGhosts/seedRangeFocusAuto in DomBackend.
           |function __kyoRangePaths(r,f){var s={};var n=r.s.nextSibling;while(n&&n!==r.e){if(n.nodeType===1){var p=f(n);for(var k in p)if(Object.prototype.hasOwnProperty.call(p,k))s[k]=true;}n=n.nextSibling;}return s;}
           |function __kyoRangeGhosts(r,surv){var out=[];var n=r.s.nextSibling;while(n&&n!==r.e){if(n.nodeType===1)out=out.concat(kyoLeavePrepare(n,surv));n=n.nextSibling;}return out;}
           |function __kyoRangeEnterSeed(r,oldSet){var n=r.s.nextSibling;while(n&&n!==r.e){if(n.nodeType===1)kyoEnterSeed(n,oldSet);n=n.nextSibling;}}
           |// seedFocusAuto seeds the FIRST new candidate below the root it is handed, so it runs on exactly one
           |// range element: the first carrying a new candidate. Per-element would seed one focus per sibling.
           |function __kyoSeedRangeFocus(r,oldSet){var n=r.s.nextSibling;while(n&&n!==r.e){if(n.nodeType===1){var p=focusAutoPaths(n);for(var k in p){if(Object.prototype.hasOwnProperty.call(p,k)&&!oldSet[k]){seedFocusAuto(n,oldSet);return;}}}n=n.nextSibling;}}
