    ...
    final private class Emitter(schema: Document, config: CodegenConfig):
        ...
        private def emitInput(in: InputObjectTypeDefinition): GeneratedSource =
            ...
            val contents =
                s"""package ${config.packageName}
           |
           |import kyo.Schema$scalarImport
           |
        ...
        /** Render the selector object of a GraphQL union: its own phantom `Origin`
          ...
          * branch decodes to `Option[A]` (`Some` iff the object's `__typename` is
        ...
        private def renderUnionObject(u: UnionTypeDefinition): GeneratedSource =
            ...
            val branches = u.memberTypes.map { member =>
                ...
                s"""def $label[A](sel: SelectionBuilder[$member, A]): SelectionBuilder[$name, ($label: Option[A])] =
           |  SelectionBuilder.onType("$member", sel)
           |
           |def $label[A](build: SelectionBuilder[$member, scala.NamedTuple.Empty] => SelectionBuilder[$member, A]): SelectionBuilder[$name, ($label: Option[A])] =
           |  SelectionBuilder.onType("$member", build(SelectionBuilder.empty))""".stripMargin
            }.mkString("\n\n")
            val chainAccessors = u.memberTypes.map { member =>
                val label = s"on$member"
                s"""def $label[B](sub: SelectionBuilder[$member, scala.NamedTuple.Empty] => SelectionBuilder[$member, B]): SelectionBuilder[$name, scala.NamedTuple.Concat[Acc, ($label: Option[B])]] =
        ...
        private def renderSelectorObject(
            ...
            val contents =
                s"""package ${config.packageName}
           |
           |import kyo.apollo.api.*$scalarImport$clientImport
           |
        ...
        private def selectorMethod(originName: String, field: FieldSpec): String =
            ...
            val argParams = field.args.map { a =>
                val default = if isNonNull(a.ofType) then "" else " = None"
                s"${sanitize(a.name)}: ${scalaTypeOf(a.ofType)}$default"
            }
            val argList =
                if field.args.isEmpty then "Nil"
                else
            ...
            if isComposite(leafName) then
                ...
                // (`_.code.name`) identically — no more explicit `None` on the lambda.
        ...
        private def scalarCodecExpr(t: Type): String = t match
            case Type.NamedType(name, nonNull) =>
                ...
                if nonNull then leaf else s"ScalarCodec.option($leaf)"
            case Type.ListType(ofType, nonNull) =>
                val listed = s"ScalarCodec.list(${scalarCodecExpr(ofType)})"
                if nonNull then listed else s"ScalarCodec.option($listed)"
        ...
        /** The Scala value type for a field whose object leaf named tuple is `a`,
          * honouring list/nullable wrappers, e.g. `[Country!]` → `Option[List[A]]`.
          */
        private def wrappedTypeExpr(t: Type, a: String): String = t match
            case Type.NamedType(_, nonNull) => if nonNull then a else s"Option[$a]"
            case Type.ListType(ofType, nonNull) =>
                val inner = s"List[${wrappedTypeExpr(ofType, a)}]"
                if nonNull then inner else s"Option[$inner]"
        ...
        private def scalaTypeOf(t: Type): String = t match
            ...
            case Type.ListType(ofType, nonNull) =>
                wrapNullable(s"List[${scalaTypeOf(ofType)}]", nonNull)

        private def wrapNullable(scala: String, nonNull: Boolean): String =
            if nonNull then scala else s"Option[$scala]"
